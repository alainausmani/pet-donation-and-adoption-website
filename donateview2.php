...
$sql = "SELECT dp.pet_id, dp.pet_name, dp.breed, dp.age, dp.pet_type, dp.health_condition,dp.amount_left, dp.pet_image
        FROM petsdonate as dp 
        WHERE dp.statuss='PENDING'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      echo '<p><strong>Health Condition:</strong> ' . $row['health_condition'] . '</p>';
      echo '<p><strong>Amount Needed:</strong> ' . $row['amount_left'] . '</p>';
      echo '<p><a href="donateinfoforma.php?pet_id=' . $row['pet_id'] . '" class="donate_button">Donate for ' . $row['pet_name'] . '</a></p>';
      echo '</div>';
      $imagePath = "pet_images/{$row['pet_name']}_{$row['pet_id']}.jpg";
